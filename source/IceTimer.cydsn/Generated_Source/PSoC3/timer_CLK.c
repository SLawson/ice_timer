...
void timer_CLK_Start(void) 
{
    ...
    timer_CLK_CLKEN |= timer_CLK_CLKEN_MASK;
	timer_CLK_CLKSTBY |= timer_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: timer_CLK_Stop
********************************************************************************
*
* Summary:
...
void timer_CLK_Stop(void) 
{
    ...
    timer_CLK_CLKEN &= (uint8)(~timer_CLK_CLKEN_MASK);
	timer_CLK_CLKSTBY &= (uint8)(~timer_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: timer_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((timer_CLK_CLKEN & timer_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = timer_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = timer_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* timer_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(timer_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(timer_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: timer_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: timer_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = timer_CLK_GetSourceRegister();
    uint16 oldDivider = timer_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = timer_CLK_CLKEN & timer_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((timer_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: timer_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(timer_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: timer_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_CLK_MOD_SRC |= modeBitMask & (uint8)timer_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(timer_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: timer_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return timer_CLK_MOD_SRC & (uint8)(timer_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: timer_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = timer_CLK_GetDividerRegister();
    uint8 oldSrc = timer_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        timer_CLK_MOD_SRC |= CYCLK_SSS;
        timer_CLK_MOD_SRC =
            (timer_CLK_MOD_SRC & (uint8)(~timer_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: timer_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return timer_CLK_MOD_SRC & timer_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: timer_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    timer_CLK_PHASE = clkPhase & timer_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: timer_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return timer_CLK_PHASE & timer_CLK_PHASE_MASK;
}
